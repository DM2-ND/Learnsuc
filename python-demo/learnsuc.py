...
def EasyNegSinh(x):
    if x < 0.05 or x > 100.0: return 0.0
    return 1.0/x/x/x/np.sinh(1.0/x)

def GetBehaviorStr(behavior,paperid,authorid2authorname,confid2confname,paperid2titleyear):
    ret = ''
    if paperid in paperid2titleyear:
        ret += '|'+paperid2titleyear[paperid]
    s = ''
    for item in behavior[0]:
        s += ','+authorid2authorname[item]
    ret += '|'+s[1:]
    s = ''
    for item in behavior[1]:
        s += ','+confid2confname[item]
    ret += '|'+s[1:]
    s = ''
    for item in behavior[2]:
        s += ','+item
    ret += '|'+s[1:]
    return ret[1:]
